#https://raw.githubusercontent.com/fizyr/keras-retinanet/master/keras_retinanet/utils/eval.py
...
def _get_detections(generator, model, score_threshold=0.05, max_detections=100, save_path=None):
    ...
    for i in progressbar.progressbar(range(generator.size()), prefix='Running network: '):
        ...
        ximage = np.expand_dims(image, axis=0)
        #xtensor= tf.convert_to_tensor(ximage)
        
        #zz = model.predict_on_batch(image)
        #print(zz)
        #print(var(zz))
        boxes, scores, labels = model.predict_on_batch([ximage])
